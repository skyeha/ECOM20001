...
# Modified to fit on my computer
#**********************************************************************************************
...
setwd("D:/Semester 2 2022/ECOM20001/Tutorials/Week 5")

## Load Stargazer package for summart statistics and regression tables
library(stargazer)
...
height_data=read.csv(file="tute5_height.csv")
...
earn_reg1=lm(earnings~height,data=height_data)  # run the single linear regression
summary(earn_reg1)                          # summarize the regression results

# A note in passing: the summary() command for summarising the results from a regression assumes
# homoskedastic standard errors. For tutorial 5, we will work with homoeskedastic standard
# errors as we introduce inference with single linear regression. Next week, in tutorial 6, 
# we will extend our code to properly account for heteroskedastic standard errors. From that  
# point on, all of our tutorials and assignments (2 & 3) will work with heteroskedastic
# standard errors, as discussed at the end of Lecture Note 5.
...
# A. LOAD DATA
## Load dataset on income and height
crime_data=read.csv(file="tute5_crime.csv")


# B. DESCRIPTIVES

## Summary Statistics
stargazer(crime_data, 
...
plot(crime_data$police,crime_data$homicides,
...
ggplot(crime_data, aes(police, homicides)) + geom_point()
