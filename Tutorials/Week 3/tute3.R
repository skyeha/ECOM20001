...
# C. SUBSETS OF DATA BASED ON MULTIPLE VARIABLES
...
## A. DESCRIPTIVE STATISTICS FOR ENTIRE DATASET
stargazer(data, 
...
# Here, we use the summary() and sd() commands to obtain all of the relevant
# summary statistics for different subgroups of data. This does not immediately
# provide a "nice" table like stargazer does, but the code is much simpler
# than what is required for stargazer for looking at summary statistics
# for subsets of data

## Compare means and standard deviation for ahe for females and males
# Females
summary(data$ahe[data$female==1])  # summary(): summary statistics except std dev.
sd(data$ahe[data$female==1])       # sd(): std dev.
...
#**********************************************************************************************
# 4. TESTING MEANS AND CONFIDENCE INTERVALS: t.test(), length(), pnorm(), abs()
...
# To compute the p-value we use the pnorm() command, where pnorm(X) returns the 
# value of the cumulative density function for the N(0,1) distribution at X
...
paste("One-sided p-value for less than (<) alternative:",pvalue2)

# p-value for the two-sided test (just for completeness)
# Here, we also use the abs() command for taking an absolute value, where
# abs(X) returns the absolute value of X
t_act=(ahe_mu-19.5)/ahe_se  # t-statistic
pvalue3=2*pnorm(-abs(t_act))        # compute p-value
paste("Two-sided p-value for equals (=) alternative:",pvalue3)
